...
def FileListing(TargetDirectory):
    """
    Retrieve directory listing of *.rslt files to be translated
    ...
    for file in os.listdir(TargetDirectory):
        if file.endswith('.rslt'):
            inSpecraFiles.append(file)
    return inSpecraFiles


def retrieve_GADRAS_Results(filepath):
    """
    ...
    with open(filepath) as resFile:
        results = resFile.read().split('Isotopes:')[1].strip().split(' + ')
    ...
    for res in results:
        ...
        if len(result) == 2:
            resultsArray.append([result[0].strip(' '), result[1].strip(')')])
        else: resultsArray.append([result[0], '0'])
...
def write_GADRAS_results(resultsArray, outFilepath):
    """
...
def main(input_dir, output_dir, template_dir="./"):
    ...
    for fname in inSpecraFiles:
        ResultsArray = retrieve_GADRAS_Results(os.path.join(input_dir, fname))

        newFileName = os.path.join(output_dir, fname.replace('.rslt', '.res'))

        write_GADRAS_results(ResultsArray, newFileName)
